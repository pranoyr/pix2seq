...
class Pix2SeqTokenizer:
    ...
    def decode(self, token_ids):
        ...
        # Remove BOS/EOS/PAD
        clean_tokens = [
            t for t in token_ids if t not in [self.bos_id, self.eos_id, self.pad_id]
        ]
        ...
        for chunk in chunks:
            ...
            if not (self.coord_start_id <= y1_id < self.coord_end_id):
                continue
...
if __name__ == "__main__":
    ...
    print(
        f"Class Range: {tokenizer.class_start_id} - {tokenizer.class_start_id + len(coco_classes)}"
    )
    ...
    for box in decoded_boxes:
        ...
        unnormalized_boxes.append(
            [x * orig_size[1], y * orig_size[0], w * orig_size[1], h * orig_size[0]]
        )
